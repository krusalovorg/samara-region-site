...
function PlaceItem({data}: {data?: any}) {
    ...
    return (
        <div
            ...
            style={{
                backgroundImage: `url(${data?.image || ImageCard2})`,
            ...
            <div className='px-[5%] py-[20px] h-full w-full flex z-[100] relative flex flex-col'>
                ...
                <h2 className='text-white font-bold spacing-[0px] text-3xl mt-auto'>{data?.name || "Замок \"Гарибальди\""}</h2>
                <h3 className='text-white spacing-[0px] text-lg mt-1'>{data?.description || "20км от Ягодного · Длительность 1 час"}</h3>
            </div>
        </div>
    )
}
